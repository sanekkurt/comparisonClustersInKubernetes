...
func CompareNodeSelectors(ctx context.Context, nodeSelector1, nodeSelector2 map[string]string) {
	...
	for key1, value1 := range nodeSelector1 {
		...
		for key2, value2 := range nodeSelector2 {
			...
			if value1 == value2 && key1 == key2 {
				...
				diffsBatch.Add(ctx, false, "%w. '%s-%s' vs '%s-%s'", ErrorDiffersNodeSelectorsInTemplates, key1, value1, key2, value2)
				flag = true
				delete(nodeSelector1, key1)
				delete(nodeSelector2, key1)
				break
		...
		if !flag {
			...
			diffsBatch.Add(ctx, false, "%w. Cluster number: 2. %s-%s ", ErrorNodeSelectorsDoesNotExist, key1, value1)
			delete(nodeSelector1, key1)
		}
	}
	...
	if len(nodeSelector2) != 0 {
		for key, value := range nodeSelector2 {
			...
			diffsBatch.Add(ctx, false, "%w. Cluster number: 1. %s-%s ", ErrorNodeSelectorsDoesNotExist, key, value)
