...
func compareContainerSpecImages(ctx context.Context, container1, container2 v1.Container) error {
	...
	if imgParts[0][0] != imgParts[1][0] {
		...
		diffsBatch.Add(ctx, false, "%w: %s vs %s", ErrorContainerDifferentImageLabels, imgParts[0][0], imgParts[1][0])
	}

	if imgParts[0][1] != imgParts[1][1] {
		//log.Warnf("%s: %s vs %s", ErrorContainerDifferentImageTags.Error(), imgParts[0][1], imgParts[1][1])
		diffsBatch.Add(ctx, false, "%w: %s vs %s", ErrorContainerDifferentImageTags, imgParts[0][1], imgParts[1][1])
	}
	...
	if container1.ImagePullPolicy != container2.ImagePullPolicy {
		...
		diffsBatch.Add(ctx, false, "%w: %s vs %s", ErrorContainerDifferentImagePolicies, container1.ImagePullPolicy, container2.ImagePullPolicy)
	}
