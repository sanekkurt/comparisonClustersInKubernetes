...
func compareCommonProbeParams(ctx context.Context, probe1, probe2 v1.Probe) {
	...
	if probe1.FailureThreshold != probe2.FailureThreshold {
		...
		diffsBatch.Add(ctx, false, "%w: '%d' vs '%d'", ErrorContainerHealthCheckDifferentFailureThreshold, probe1.FailureThreshold, probe2.FailureThreshold)
	}
	...
	if probe1.InitialDelaySeconds != probe2.InitialDelaySeconds {
		...
		diffsBatch.Add(ctx, false, "%w: %d vs %d", ErrorContainerHealthCheckDifferentInitialDelaySeconds, probe1.InitialDelaySeconds, probe2.InitialDelaySeconds)
	}
	...
	if probe1.PeriodSeconds != probe2.PeriodSeconds {
		...
		diffsBatch.Add(ctx, false, "%w: %d vs %d", ErrorContainerHealthCheckDifferentPeriodSeconds, probe1.PeriodSeconds, probe2.PeriodSeconds)
	}
	...
	if probe1.SuccessThreshold != probe2.SuccessThreshold {
		...
		diffsBatch.Add(ctx, false, "%w: %d vs %d", ErrorContainerHealthCheckDifferentSuccessThreshold, probe1.SuccessThreshold, probe2.SuccessThreshold)
	}
	...
	if probe1.TimeoutSeconds != probe2.TimeoutSeconds {
		...
		diffsBatch.Add(ctx, false, "%w: %d vs %d", ErrorContainerHealthCheckDifferentTimeoutSeconds, probe1.TimeoutSeconds, probe2.TimeoutSeconds)
	}
}
