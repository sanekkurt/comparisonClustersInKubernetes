...
func compareExecProbes(ctx context.Context, probe1, probe2 v1.Probe) {
	...
	if probe1.Exec != nil && probe2.Exec != nil {
		if bDiff, dif := utils.AreStringListsEqual(ctx, probe1.Exec.Command, probe2.Exec.Command); !bDiff {
			...
			diffsBatch.Add(ctx, &meta.Type, &meta.Meta, false, zap.WarnLevel, "%s: %s", ErrorContainerHealthCheckDifferentExecCommand.Error(), dif)
		}
		...
		diffsBatch.Add(ctx, &meta.Type, &meta.Meta, false, zap.WarnLevel, "%s: %s vs %s", ErrorContainerHealthCheckDifferentExec, probe1.Exec, probe2.Exec)
	}
