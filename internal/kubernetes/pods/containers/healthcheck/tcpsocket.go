...
func compareTCPSocketProbes(ctx context.Context, probe1, probe2 corev1.Probe) {
	...
	if probe1.TCPSocket != nil && probe2.TCPSocket != nil {
		if probe1.TCPSocket.Host != probe2.TCPSocket.Host {
			...
			diffsBatch.Add(ctx, false, "%s: %s vs %s", ErrorContainerHealthCheckDifferentTCPSocketHost.Error(), probe1.TCPSocket.Host, probe2.TCPSocket.Host)
		}
		...
		if probe1.TCPSocket.Port.IntVal != probe2.TCPSocket.Port.IntVal ||
			...
			diffsBatch.Add(ctx, false, "%s: %s vs %s", ErrorContainerHealthCheckDifferentTCPSocketPort.Error(), probe1.TCPSocket.Port, probe2.TCPSocket.Port)
		}
		...
		diffsBatch.Add(ctx, false, "%s", ErrorContainerHealthCheckDifferentTCPSocket.Error())
	}
}
